...
{
    // Condensed version of pcg_output_rxs_m_xs_32_32, with simple conversion to floating-point
    // [0,1].
...
void main()
{
    ...
    for (int sampleIdx = 0; sampleIdx < NUM_SAMPLES; sampleIdx++)
    {
        ...
        //    '------+------'
        //          -1
        const vec2 randomPixelCenter =
            vec2(pixel) + vec2(stepAndOutputRNGFloat(rngState), stepAndOutputRNGFloat(rngState));
        const vec2 screenUV =
            vec2((2.0 * randomPixelCenter.x - resolution.x) / resolution.y,   //
                 -(2.0 * randomPixelCenter.y - resolution.y) / resolution.y); // Flip the y axis
        // Create a ray direction:
        vec3 rayDirection =
            vec3(fovVerticalSlope * screenUV.x, fovVerticalSlope * screenUV.y, -1.0);
        rayDirection = normalize(rayDirection);

        vec3 accumulatedRayColor =
            vec3(1.0); // The amount of light that made it to the end of the current ray.
        ...
        for (int tracedSegments = 0; tracedSegments < 32; tracedSegments++)
        {
                ...
                gl_RayQueryCommittedIntersectionTriangleEXT)
            {
                ...
                hitInfo.worldNormal =
                    faceforward(hitInfo.worldNormal, rayDirection, hitInfo.worldNormal);
                ...
                const float theta =
                    6.2831853 * stepAndOutputRNGFloat(rngState);             // Random in [0, 2pi]
