...
{
    _resourceMap[static_cast<size_t>(AccessType::eReadOnly)].emplace_back(texture, setIdx, bindIdx,
                                                                          offset);
}

void RDGPass::addRead(RDGBuffer* buffer, uint32_t setIdx, uint32_t bindIdx, uint32_t offset)
{
    _resourceMap[static_cast<size_t>(AccessType::eReadOnly)].emplace_back(buffer, setIdx, bindIdx,
                                                                          offset);
}

void RDGPass::addReadWrite(RDGTexture* texture, uint32_t setIdx, uint32_t bindIdx, uint32_t offset)
{
    _resourceMap[static_cast<size_t>(AccessType::eReadWrite)].emplace_back(texture, setIdx, bindIdx,
                                                                           offset);
}

void RDGPass::addReadWrite(RDGBuffer* buffer, uint32_t setIdx, uint32_t bindIdx, uint32_t offset)
{
    _resourceMap[static_cast<size_t>(AccessType::eReadWrite)].emplace_back(buffer, setIdx, bindIdx,
                                                                           offset);
}
...
void RDGRenderPass::depthStencilAttach(RDGRTState::LoadType  loadType,
                                       RDGRTState::StoreType storeType, RDGTexture* texture,
                                       VkImageLayout initLayout, VkImageLayout finalLayout)
{
