...
RDGBuffer* RDGBufferPool::alloc()
{
    ...
    if (this->_availableIndex >= this->_objs.size())
    {
        throw std::runtime_error(
            "RDGBufferDescriptionPool: No available buffer description in pool");
    }
    uint32_t index     = this->_freeIndices[this->_availableIndex++];
...
RDGBuffer* RenderDependencyGraph::createBuffer(const BufferDesc& desc)
{
    ...
    buffer->_rhi->BufferProperty() =
        desc._location == BufferDesc::MemoryLocation::eDeviceLocal
...
void RenderDependencyGraph::allocRHITexture(RDGTexture* texture)
{
    ...
    if (texture->_rhi->UsageFlags() & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT)
    {
        ...
        switch (texture->_rhi->Type())
        {
            ...
            {
                viewInfo.viewType = (texture->_rhi->LayerCount() > 1 ? VK_IMAGE_VIEW_TYPE_1D_ARRAY
                                                                     : VK_IMAGE_VIEW_TYPE_1D);
                break;
            }
            case VK_IMAGE_TYPE_2D:
            {
                viewInfo.viewType = (texture->_rhi->LayerCount() > 1 ? VK_IMAGE_VIEW_TYPE_2D_ARRAY
    ...
    else
    {
        ...
        texture->_rhi = TexturePool::Instance().alloc(imgInfo.extent.width, imgInfo.extent.height,
                                                      imgInfo.extent.depth, imgInfo.format,
                                                      imgInfo.usage, imgInfo.initialLayout);
        NVVK_DBG_NAME(texture->_rhi->image);
    }
    return;
}
...
//         contextRtState.emplace_back(static_cast<uint8_t>(stat.loadType),
//         static_cast<uint8_t>(stat.storeType),
...
void RenderDependencyGraph::allocRHIBuffer(RDGBuffer* buffer)
{
    ...
    BufferPool::Instance().alloc(buffer->_rhi->BufferSize(), buffer->_rhi->UsageFlags(),
                                 buffer->_rhi->BufferProperty());
    return;
}
                                      ...
                                      int currDepth)
{
    ...
    visited.insert(pass);
    _passDepthLayout[pass->_passID.value()] =
        std::max(_passDepthLayout[pass->_passID.value()], currDepth);
