...
{
    if(reader.num_rows() != 1 || reader.element() == nullptr || reader.element()->properties.size() < count)
        return false;
...
bool extractIndexedProperties(miniply::PLYReader& reader,
                              const miniply::PLYElement* elem,
                              const char*                prefix,
                              uint32_t                   count,
                              PLYPropertyType            destType,
                              void*                      dst)
{
    if(reader.num_rows() != 1 || elem == nullptr)
        return false;
...
bool extractNamedPairU32(miniply::PLYReader& reader, const miniply::PLYElement* elem, uint32_t outPair[2])
{
    ...
    if(widthIdx == kInvalidIndex || heightIdx == kInvalidIndex)
        return false;
...
bool extractNamedPairF32(miniply::PLYReader& reader,
                         const miniply::PLYElement* elem,
                         const char*                a,
                         const char*                b,
                         float                      outPair[2])
{
    ...
    if(idxA == kInvalidIndex || idxB == kInvalidIndex)
        return false;
...
bool isMetaCandidate(const miniply::PLYElement* elem)
{
    ...
    if(elem->name == "extrinsic" || elem->name == "intrinsic" || elem->name == "image_size" || elem->name == "frame"
       || elem->name == "disparity" || elem->name == "color_space" || elem->name == "colorspace" || elem->name == "version")
        return true;
...
void parseMetaFromCurrentElement(miniply::PLYReader& reader, const miniply::PLYElement* elem, GaussianSceneMeta& meta)
{
    ...
    if(elem->name == "image_size")
    {
        extractIndexedProperties(reader, elem, "image_size", 2, PLYPropertyType::UInt, meta.imageSize)
            ...
            || extractFirstProperties(reader, 2, PLYPropertyType::UInt, meta.imageSize);
        return;
    }
    if(elem->name == "frame")
    {
    ...
    if(csIdx != kInvalidIndex && reader.num_rows() == 1)
        reader.extract_properties(&csIdx, 1, PLYPropertyType::UChar, &meta.colorSpace);
...
bool GaussianScene::load(const std::filesystem::path& filename)
{
    ...
    while(reader.has_element())
    {
        ...
        if(isVertexElement)
        {
            ...
            if(!reader.find_properties(propIdxs.data(), static_cast<uint32_t>(propIdxs.size()), "x", "y", "z", "f_dc_0", "f_dc_1",
                                       "f_dc_2", "opacity", "scale_0", "scale_1", "scale_2", "rot_0", "rot_1", "rot_2", "rot_3"))
            {
                _vertices.clear();
                _meta = {};
                return false;
            ...
            if(!reader.extract_properties_with_stride(propIdxs.data(),
                                                      static_cast<uint32_t>(propIdxs.size()),
                                                      PLYPropertyType::Float,
                                                      _vertices.data(),
                                                      static_cast<uint32_t>(sizeof(GaussianVertex))))
            {
                _vertices.clear();
                _meta = {};
                return false;
