...
FixedMaterial* FixedMaterial::Create()
{
    ...
    static RefPtr<RenderProgram> commonFixedProgram = RefPtr<RenderProgram>(new RenderProgram(vertexShaderID, fragShaderID));
    ...
    if (!fixedMaterial._program)
    {
        ...
        commonFixedProgram->psoState().colorBlendEquations.resize(GBufferConfig::RT_COUNT, {
        ...
        fixedMaterial._program                                     = commonFixedProgram.get();
    }
