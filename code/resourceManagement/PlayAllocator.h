...
public:
    ...
    // 立方体纹理分配
   [[nodiscard]]Texture* allocCube(uint32_t size, VkFormat format,
                      VkImageUsageFlags usage, VkImageLayout initialLayout,
                      uint32_t mipLevels = 1);

    // 从像素数据分配2D纹理
    [[nodiscard]]Texture* alloc(const void* data, size_t dataSize,
    ...
    // 通过文件路径创建2D纹理
    [[nodiscard]]Texture* alloc(const std::filesystem::path& imagePath,
                   VkImageLayout finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                   uint32_t mipLevels = 1){return nullptr;}
...
public:
    ...
    // Buffer* alloc(const std::filesystem::path& filePath, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties);
...
class PlayResourceManager: protected nvvk::ResourceAllocatorExport,protected nvvk::StagingUploader,public nvvk::SamplerPool
{
public:
