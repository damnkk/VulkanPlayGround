...
public:
    static Texture* Create(uint32_t width, uint32_t height, VkFormat format,
                   VkImageUsageFlags usage, VkImageLayout layout = VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    static Texture* Create(uint32_t size, VkFormat format,
                      VkImageUsageFlags usage, VkImageLayout initialLayout,
                      uint32_t mipLevels = 1);
    ...
    int                     Id(){ return poolId;}
    VkFormat&               Format()  { return format; }
    VkImageType&            Type()  { return type; }
    VkExtent3D&             Extent()  { return extent; }
    VkSampleCountFlagBits&  SampleCount()  { return sampleCount; }
    std::string&            DebugName()  { return debugName; }
    uint32_t&               MipLevel()  { return mipLevels; }
    bool                    isDepth() const { return aspectFlags &(VK_IMAGE_ASPECT_DEPTH_BIT|VK_IMAGE_ASPECT_STENCIL_BIT); }
    VkImageLayout&          Layout(){return descriptor.imageLayout;}
    VkImageUsageFlags&      UsageFlags(){return usageFlags;}
    VkImageAspectFlags&     AspectFlags(){return aspectFlags;}
    uint32_t&               LayerCount(){return arrayLayers;}
...
public:
    ...
    std::string&         DebugName() {return debugName;}
    VkBufferUsageFlags&  UsageFlags() {return usageFlags;}
    VkDeviceSize&       BufferSize() {return bufferSize;}
    VkDeviceSize&       BufferRange() {return range;}
    VkMemoryPropertyFlags& BufferProperty() {return property;}
